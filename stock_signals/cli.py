...
def print_text_report(result, code):
    ...
    if summary:
        ...
        for desc in summary.get("signals", []):
...
def main():
    ...
    # --- sector subcommand ---
    p_meme = sub.add_parser("meme", help="猫姐 meme 股票追踪")
    meme_sub = p_meme.add_subparsers(dest="meme_cmd")
    meme_sub.add_parser("list", help="查看 watchlist")
    p_add = meme_sub.add_parser("add", help="添加股票")
    p_add.add_argument("codes", nargs="+", help="股票代码")
    p_add.add_argument("--source", default="manual")
    p_add.add_argument("--note", default="")
    p_rem = meme_sub.add_parser("remove", help="移除股票")
    p_rem.add_argument("codes", nargs="+", help="股票代码")
    meme_sub.add_parser("scan", help="分析所有 meme 股票")
    meme_sub.add_parser("scrape", help="尝试自动抓取")
    ...
    logger.info("stock-signals v2.14.0 启动")
