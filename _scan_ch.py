...
top=results[:5]
print(f'扫描 {len(codes)} 只 | 通过 {len(results)} 只')
for i,r in enumerate(top,1):
    print(f"  {i}. {r['code']} {r['rating_cn']}({r['rating']}) 分={r['score']} RR={r['risk_reward']}:1 {r['trend_phase_cn']}({r['trend_phase']})")
    print(f"     现价:{r['last_close']:.2f} 入场:{r['entry']:.2f} 止损:{r['stop_loss']:.2f} 目标1:{r['target_1']:.2f}")
with open(r'D:\Backup\Documents\ChatGPT\AI\stock-signals\scan_result.json','w',encoding='utf-8') as f:
    json.dump({'date':'2026-08-19','summary':{'scan_time':time.strftime('%Y-%m-%d %H:%M:%S'),'total_analyzed':len(codes),'total_picks':len(results)},'picks':{'US':results}},f,indent=2,ensure_ascii=False)
print('已保存')
